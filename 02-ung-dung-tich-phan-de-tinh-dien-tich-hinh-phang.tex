...
\title{2. ỨNG DỤNG TÍCH PHÂN ĐỂ TÍNH DIỆN TÍCH HÌNH PHẲNG}
\date{}
...
\section*{a) Hình phẳng giới hạn bởi một đồ thị hàm số, trục hoành và hai đường thẳng $x = a, x = b$}
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        % Đồ thị parabol y = x^2 - 4 (màu hồng/đỏ)
        \addplot[name path=f, magenta, domain=-1:3.3] {x^2 - 4};
        % Trục hoành để làm mốc tô màu
        \addplot[name path=axis, draw=none, domain=-1:3.3] {0};
        
        % Tô màu cam phần diện tích giới hạn từ x = 0 đến x = 3
        \addplot[orange!60] fill between[of=f and axis, domain=0:3];
        
        % Chỉ giữ lại đường gióng nét đứt phía trên ở x = 3, bỏ đoạn thừa ở x = 2
        \draw[dashed, thick] (axis cs:3,0) -- (axis cs:3,5);
        
        % Nhãn tên hàm y = x^2 - 4 nằm dọc theo đồ thị giống SGK
...
\section*{b) Hình phẳng giới hạn bởi hai đồ thị hàm số và hai đường thẳng $x = a, x = b$}
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        % Đồ thị y = 4 - x^2 (magenta) và y = x^2 (blue)
        ...
        % Các đường gióng đứt đoạn
        \draw[dashed, thick] (axis cs:-1,0) -- (axis cs:-1,3);
        \draw[dashed, thick] (axis cs:1,0) -- (axis cs:1,3);
        
        % Đặt nhãn sát đuôi đồ thị đỏ và đẩy nhãn xanh cao lên
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        % Đồ thị y = sin(x) (blue) và y = cos(x) (red)
        ...
        % Đường gióng nét đứt tại pi/4
        \draw[dashed, thick] (axis cs:0.7854,0) -- (axis cs:0.7854,0.707);
        
        % Nhãn sin(x) ở trên và dịch nhãn cos(x) xuống hẳn phía dưới vùng trống bên phải
